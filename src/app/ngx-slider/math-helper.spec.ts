...
describe('MathHelper', () => {
  ...
  describe('isModuloWithinPrecisionLimit', () => {
    it('works for decimal modulo', () => {
      expect(2 % 0.1 === 0).toBeFalse();
      expect(MathHelper.isModuloWithinPrecisionLimit(2, 0.1, 10)).toBeTrue();

      expect(MathHelper.isModuloWithinPrecisionLimit(2 - 1e-12, 0.1, 10)).toBeTrue();
      expect(MathHelper.isModuloWithinPrecisionLimit(2 + 1e-12, 0.1, 10)).toBeTrue();

      expect(MathHelper.isModuloWithinPrecisionLimit(2, 0.1 - 1e-12, 10)).toBeTrue();
      expect(MathHelper.isModuloWithinPrecisionLimit(2, 0.1 + 1e-12, 10)).toBeTrue();

      expect(MathHelper.isModuloWithinPrecisionLimit(2 + 1e-9, 0.1, 10)).toBeFalse();
      expect(MathHelper.isModuloWithinPrecisionLimit(2 - 1e-9, 0.1, 10)).toBeFalse();
      expect(MathHelper.isModuloWithinPrecisionLimit(2, 0.1 - 1e-9, 10)).toBeFalse();
      expect(MathHelper.isModuloWithinPrecisionLimit(2, 0.1 + 1e-9, 10)).toBeFalse();
    });
  });
